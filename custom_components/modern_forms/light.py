...
async def async_setup_entry(
    ...
    entities: list[LightEntity] = [
        ModernFormsLightEntity(entry_id=config_entry.entry_id, coordinator=coordinator)
    ]

    if coordinator.modern_forms.has_uplight():
        entities.append(
            ModernFormsUplightEntity(
...
class ModernFormsUplightEntity(ModernFormsDeviceEntity, LightEntity):
    """Defines a Modern Forms uplight."""

    _attr_min_color_temp_kelvin = LIGHT_COLOR_TEMP_MIN_KELVIN
    _attr_max_color_temp_kelvin = LIGHT_COLOR_TEMP_MAX_KELVIN
    _attr_translation_key = "uplight"

    def __init__(
        self, entry_id: str, coordinator: ModernFormsDataUpdateCoordinator
    ) -> None:
        """Initialize Modern Forms uplight."""
        super().__init__(
            entry_id=entry_id,
            coordinator=coordinator,
        )
        self._attr_unique_id = f"{self.coordinator.data.info.mac_address}_uplight"

    @property
    def color_mode(self) -> ColorMode:
        """Return the current color mode."""
        if self.coordinator.data.state.uplight_color_temp_kelvin is not None:
            return ColorMode.COLOR_TEMP
        return ColorMode.BRIGHTNESS
    ...
    def brightness(self) -> int | None:
        """Return the brightness of this light between 1..255."""
        if self.coordinator.data.state.uplight_brightness is None:
            return None
        return round(
            percentage_to_ranged_value(
                BRIGHTNESS_RANGE, self.coordinator.data.state.uplight_brightness
            )
        )
    ...
    def color_temp_kelvin(self) -> int | None:
        """Return the color temperature in Kelvin."""
        return self.coordinator.data.state.uplight_color_temp_kelvin

    @property
    def is_on(self) -> bool:
        """Return the state of the light."""
        return bool(self.coordinator.data.state.uplight_on)

    @modernforms_exception_handler
    async def async_turn_off(self, **_kwargs: Any) -> None:
        """Turn off the light."""
        await self.coordinator.modern_forms.uplight(on=LIGHT_POWER_OFF)
